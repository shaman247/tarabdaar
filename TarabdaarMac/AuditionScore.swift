...
/// Watches the auditions inbox (see `resolveAuditionsRoot`) for `.json` scores and
...
final class AuditionRunner: ObservableObject {
    ...
    ///   1. `TARABDAAR_AUDITIONS_DIR` env var (absolute path)
    ...
    ///      through to (3).
    ///   3. `~/Library/Application Support/Tarabdaar/Auditions/` — sane
    ///      fallback for any non-dev case.
    private static func resolveAuditionsRoot() -> URL {
        if let env = ProcessInfo.processInfo.environment["TARABDAAR_AUDITIONS_DIR"],
           !env.isEmpty {
            return URL(fileURLWithPath: env, isDirectory: true)
        }
        let here = URL(fileURLWithPath: #filePath)
        // <repo>/TarabdaarMac/AuditionScore.swift  → up twice → <repo>
    ...
    private func beginWatching() {
        ...
        guard dirFD >= 0 else {
            NSLog("Tarabdaar audition: failed to open \(inboxURL.path) for watching")
            return
        }
    ...
    private func apply(event: AuditionEvent) {
        ...
        default:
            NSLog("Tarabdaar audition: unknown kind \(event.kind)")
        }
    }
