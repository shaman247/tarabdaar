    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                Divider()
                DroneMappingSection(degreeLabels: degreeLabels)
            }
            .padding(16)
...
private struct DroneMappingSection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text("Each Fret Pad drone button plucks ONE of the sympathetic strings above — map up to 3. A mapped string sounds exactly as its row is tuned (pitch · gain · t60); there is no separate drone string. If the row is disabled — or the jawari selection doesn't pick it up — the button is silent. Regenerating the bank re-runs the automatic mapping (low Sa · low Pa · Sa).")
