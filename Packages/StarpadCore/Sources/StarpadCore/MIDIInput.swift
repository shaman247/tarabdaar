...
/// Starpad-owned parameters (sym pool + FX) from incoming controllers.
public final class MIDIInput: ObservableObject {
    ...
    public func start() {
        if client != 0 { return }
        let cs = MIDIClientCreateWithBlock("Starpad MIDI In" as CFString, &client) { [weak self] _ in
            self?.connectAllSources()
        }
        guard cs == noErr else {
            NSLog("Starpad: MIDIInput client create failed: \(cs)")
            return
        }
        let ps = MIDIInputPortCreateWithBlock(client, "Starpad In" as CFString, &port) { [weak self] packetList, _ in
            self?.handle(packetList: packetList)
        }
        guard ps == noErr else {
            NSLog("Starpad: MIDIInput port create failed: \(ps)")
            return
        }
        connectAllSources()
    }
    ...
    /// channel-voice messages. CoreMIDI guarantees each packet contains
    /// whole messages. SysEx isn't used by Starpad.
    private func parseBytes(_ bytes: UnsafeBufferPointer<UInt8>) {
        ...
        while i < bytes.count {
            ...
            case 0xA0, 0xC0:
                ...
                // Not used by Starpad's iPad output; skip safely.
                i += (status == 0xC0 ? 2 : 3)
            case 0xF0:
                // System-common / system-realtime. Skip the single byte.
                i += 1
            default:
                i += 1
            }
    ...
    private func handleCC(channel: UInt8, cc: UInt8, value: UInt8) {
        ...
        case 38:
            // Data LSB — Starpad sends 0 here, ignore.
            break
        default:
            break
        }
