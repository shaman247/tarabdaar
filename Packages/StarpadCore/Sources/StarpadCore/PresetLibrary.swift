...
/// hood each preset is one `.starpad` file (`StarpadPreset`) in
/// `Application Support/Starpad/Presets/`, named after the preset — so a
...
public struct PresetLibrary {
    ...
    /// The shipping location: `Application Support/Starpad/Presets/`
    ...
    public func names() -> [String] {
        ...
        return urls.filter { $0.pathExtension == "starpad" }
    ...
    @discardableResult
    public func save(_ preset: StarpadPreset, name: String) throws -> URL {
        var p = preset
    ...
    public func load(name: String) throws -> StarpadPreset {
        try StarpadPreset.decode(Data(contentsOf: url(for: name)))
    }
    ...
    public func url(for name: String) -> URL {
        directory.appendingPathComponent(name)
            .appendingPathExtension("starpad")
    }
