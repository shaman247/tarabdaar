...
/// The preset document's CODABLE halves that live in TarabdaarCore —
/// composites and tilt bindings. `TarabdaarPreset` itself lives in the Mac
...
final class PresetCodingTests: XCTestCase {
    ...
    func testSectionsSummaryDescribesWhatIsInTheFile() throws {
        let p = try TarabdaarPreset.decode(fullPreset().encoded())
    ...
    func testLegacySarangiFileStillOpens() throws {
        ...
        let p = try TarabdaarPreset.decode(data)
    ...
    /// (`tarabdaar.sarangiState.v8`) and every `.tarabdaar`/`.sarangi` file on
    ...
    /// tag and a `.tarabdaarmap` file held only composites + tilt bindings.
