    ...
    public static let groups: [(name: String, params: [ParamSpec])] = [
        ...
        ("Jawari taraf (modal contact)", [
            ...
            ParamSpec("bow_jt_drive_term", "termination drive", group: "Jawari taraf (modal contact)",
                      0.0, 1.0, 0.0, apply: .live,
                      help: "Where the played string's bridge force ENTERS each sympathetic string. 0 = the fitted 0.90 L tap, which carries a |sin(k·0.9π)| comb — modes 10 and 20 are never charged at all. 1 = the termination itself: the rows share the bridge with the played string, so a moving end excites mode k through the mode slope there, ∝ (−1)^k·k — no null anywhere, the high partials get the drive the tap's comb withheld, and the sign convention matches the pin-force radiation term the same rows already radiate. Mode 1 is held at the tap's level either way, so the fitted recruitment loudness stands. 0 = bypass, bit-exact. Applies live, slewed ~40 ms per row."),
