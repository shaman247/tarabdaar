...
///    clock, settle envelope, vibrato phase, drift walk, blend
...
/// family). ONE vocabulary: the Parameters tab's row descriptions and
/// the generated docs/parameters.md legend both render these labels and
/// texts, so the app can never tell a different timing story than the
/// documentation. (The 2026-08-23 `bow_gain` lesson: the app's old
/// apply-strategy-only text claimed "crossfaded rebuild" for all 77
/// in-place keys.)
...
public struct ParamSpec: Identifiable {
    ...
    /// slow re-mount (the tanpura register/scale-shape family schedules a
    /// seconds-long debounced tanpura rebuild). nil = derive from the
    /// apply strategy + `inPlaceKeys` (see `timing`).
...
public enum ParamRegistry {
    ...
    public static let groups: [(name: String, params: [ParamSpec])] = [
        ...
        ("Body (formula modes)", [
            ...
            ParamSpec("bow_body_tail_n", "formant modes", group: "Body (formula modes)",
                      0, 48, 32, step: 1,
                      help: "Diffuse mid/high mode forest between the tail corners — the FIXED body formants the harmonics sweep through during a glide (the cue that separates a real slide from a pitch-shifted tone; without them the radiated transfer above the signature modes measured ±0.3 dB flat — 2026-08-24). √n-normalized: more modes = denser, individually shallower structure at held total power. 0 = flat feedthrough only."),
            ...
            ParamSpec("bow_body_tail_y", "formant mobility", group: "Body (formula modes)",
                      0.0, 1.5, 0.4,
                      help: "Bridge-load side of the formant modes. Kept modest: the shipped 0.4 leaves the admittance max (and the loop cap) unchanged — raising it far invites wolves."),
            ParamSpec("bow_body_tail_rad", "formant radiation", group: "Body (formula modes)",
                      0.0, 8.0, 2.5,
                      help: "How loudly the formant forest radiates against the flat floor. Shipped 2.5 ≈ ±4 dB ripple std across 250–6500 Hz (extremes ~26 dB) — real-body territory; the pre-2026-08-24 0.017 was inaudibly flat."),
        ...
        ("Bow & string", [
            ...
            ParamSpec("bow_loss_reg", "register damping", group: "Bow & string",
                      0.0, 1.5, 0.7,
                      help: "Register-tracking string loss: below the tonic, the nut/bridge/gut loss corners scale down with pitch — fc × (f0/tonic)^this — so a low note's Helmholtz corner rounds in proportion to its period instead of staying absolutely as sharp as the fitted register's. Fixes the brassy low register (measured ~30 dB excess relative 3–6 kHz an octave below the tonic, and a hollowed fundamental that this law recovers); glides darken smoothly on the way down. At/above the tonic: untouched. 0 = fixed corners (the pre-2026-08-24 sound); the shipped 0.7 is a moderate warmth, 1.0 full period-proportional tracking."),
        ...
        ("Jawari taraf (modal contact)", [
            ParamSpec("bow_jt_gain", "level", group: "Jawari taraf (modal contact)",
                      0.0, 3.0, 0.3,
                      help: "Output mix of the jawari web (the gap-law calibration landed at 0.3). Glides ~40 ms inside the kernel (2026-09-03): the bridge-force radiation carries the contact pulse train's low-frequency swing, and a stepped gain on it clicked."),
            ...
            ParamSpec("bow_jt_apex", "graze depth", group: "Jawari taraf (modal contact)",
                      2e-6, 5e-5, 1e-5,
                      help: "Bone protrusion. The evolution lives at the grazing knee — deep press linearizes (sparkle only), too shallow never engages. (Two bridges since 2026-09-02: this group is the RAGA set's bridge plus the web-wide taraf controls; the CHROMATIC set has its own bridge group below.)"),
            ParamSpec("bow_jt_zone", "contact zone (m)", group: "Jawari taraf (modal contact)",
                      0.002, 0.02, 0.006,
                      help: "Length of the bone the string can touch, in metres — the flat of the jawari. The fitted 6 mm concentrates the contact grid on the active region (J8z6); wider = a flatter, more open jawari whose wrap spreads along the bone (longer, more diffuse buzz), narrower = a sharper knee."),
            ...
            ParamSpec("bow_jt_ev_reg", "evolution register", group: "Jawari taraf (modal contact)",
                      ...
                      help: "Register tilt of the evolution axis, in evolve units per OCTAVE from the tonic — each row's bone evaluates the same margin map at its own shifted evolve (per-row offsets, kernel-slewed ~40 ms like the axis itself). Positive opens the BELOW-tonic rows toward the grazing band while pressing the above-tonic web closed: the low Sa/Pa anchor rows — kin to nearly every note, longest t60 — sit in the cascading band for their whole ring, the sustained sarod-drone bloom, without the whole web buzzing. Negative reverses it (highs shimmer, lows stay put). 0 = the uniform bone, byte-null. Rides the evolution knob: the tilt is relative to wherever it sits. Applies live."),
            ...
            // The taraf-bus compressor (2026-08-24): dynamics on the
            // radiated jt bus ONLY, applied at the split-bus merge (the
            // FX-insert point) — the played voice is untouched. Threshold
            // 0 = off = bit-exact byte-null (BusMeterTests pins the
            // split-bus path itself).
            ...
            // The voice-relative taraf cap (2026-08-31; PER STRING since
            // 2026-09-01): where the comp holds the taraf bus against a
            // FIXED threshold, the cap holds each sympathetic string
            // against the played voice's OWN level — the runaway-bloom
            // lever (high evolve lets the web feed itself past the voice).
            // Applied inside the kernel's jt tick row by row, so one
            // blooming anchor is held without ducking its neighbours.
            // Hard 0 = off = bit-exact byte-null.
            ParamSpec("bow_jt_cap", "voice cap", group: "Jawari taraf (modal contact)",
                      0.0, 1.0, 0.0, apply: .live,
                      help: "How hard EACH sympathetic string is held at or below the played voice's own level (the runaway-bloom lever — with high evolve the web can feed itself past the voice; the fixed-threshold comp can't follow a phrase's dynamics, this cap does). The ceiling is the VOICE bus's instant-attack peak envelope decaying ~7 dB/s — a string may ring on after a note (decay more slowly than the voice) but never peak above what the voice reached — times `bow_jt_cap_ratio`. Applied per string inside the kernel: one blooming anchor row is held on its own while the rest of the web stands (the earlier version limited the summed taraf bus, so one hot string ducked all of them). 0 = off, bit-exact; 1 = a hard relative limiter (the full dB overshoot is removed); between = a soft proportional lean (that fraction of the overshoot). Dimensionless ratio law — rides bow_gain and expression untouched. Note: armed hard with the voice silent, drones and the tanpura's taraf charge are held down too until the voice first sounds. Applies live."),
        ...
        // TWO BRIDGES (2026-09-02): the CHROMATIC sympathetic set (the
        // Strings tab's second table — 15 semitone strings on the fixed
        // JI grid) sits on its own bridge with its own jawari. Every knob
        // here is the chromatic twin of a `bow_jt_*` knob and lands the
        // same way (per-row baked tables reloaded in place, or the live
        // evolve setter); the web-wide taraf controls (tone, body, gov,
        // damp, comp, cap, recruitment) stay shared above. Defaults MUST
        // equal `BowTables.chromaticBridgeDefaults` — the artifact never
        // carries these keys, so the registry default IS what the engine
        // plays (TarabSetTests pins it). At rest the two bridges are the
        // same jawari — the split alone adds strings, not a new sound.
        ("Chromatic bridge (jawari taraf)", [
            ...
            ParamSpec("bow_jtc_alpha", "contact law", group: "Chromatic bridge (jawari taraf)",
                      1.0, 2.0, 1.3,
                      help: "Contact stiffness exponent of the chromatic bridge (per row in the kernel). 1.5 = Hertz (the fast sqrt path); the shipped 1.3 is the fitted raga value."),
        ...
        // THE FRET PITCH WARP (2026-08-25): the pad-geometry knob of the
        // fret field. Like `ctl_strike_window` it is a control-layer key,
        // not a voice parameter — `AppController.applyParamToVoice`
        // intercepts it before the voice routing and relays the value to
        // the iPad over JOYCON_STATE (TLP v10), where the fret field
        // resolves touch pitch through it. Bindable live (the point:
        // ride a stick between fretless meend and quantized runs).
        ("Fret pad", [
        ...
        // THE GLIDE QUEUE (2026-08-31): control-layer keys like
        // `ctl_strike_window` — `AppController.applyParamToVoice`
        // intercepts them before the voice routing and feeds
        // `AudioEngine.glideQueue` (the `GlideSequencer`). Notes that
        // OVERLAP in time chain into queued glissandi: the later onset
        // never mounts a string, the sounding voice glides to it —
        // shaped by `ctl_fret_warp` (linear → logistic, as though the
        // finger slid between two adjacent frets). Releases are never
        // deferred (the first cut's threshold window + release grace
        // sustained every staccato tap and was replaced by the overlap
        // rule the same day). Toggle 0 = off (pure pass-through, the
        // inert-default contract).
        ("Glide", [
            ParamSpec("ctl_glide_on", "glide enable", group: "Glide",
                      0.0, 1.0, 0.0, apply: .live,
                      scope: .perNote,
                      help: "The glide queue's on/off toggle (≥ 0.5 = on). On: a note played while another is still HELD does not mount a fresh string — it is QUEUED and the sounding voice glides to it; further overlapping notes join the queue and are hit in sequence, and once every chained touch has lifted the next tap is a fresh attack (staccato is untouched — releases are never deferred). A repeat tap at the sounding pitch still re-attacks. 0 = off — every onset is a fresh note, the historic behavior."),
            ParamSpec("ctl_glide_rate", "glide rate (st/s)", group: "Glide",
                      2.0, 200.0, 40.0, apply: .live,
                      scope: .perNote,
                      help: "Base speed of a queued glide, in semitones per second, when the note being left has been RELEASED. A 12-semitone glide at 40 st/s takes 0.3 s. Bindable — the legacy keyboard's tilt-driven glide speed, reborn on the registry."),
        ...
        // THE CONTROLLER STRUM (2026-08-28): control-layer keys like
        // `ctl_strike_window` — `AppController.applyParamToVoice`
        // intercepts them before the voice routing. The strum SET itself
        // lives in the sarangi document (Strings tab, string references =
        // scale degrees); the Joy-Con L button holds the chord.
        ("Controller", [
        ...
        ("Liveness", [
            ...
            ParamSpec("bow_glide_dip_db", "glide dip (dB)", group: "Liveness",
                      0, 12, 5.0,
                      scope: .perNote,
                      help: "Glide bow lightening: the bow eases while the pitch is MOVING (full depth on velocity, 0.3× on force), fitted to SWAM's 3.5–9 dB transition dips. Since every note-on mounts a fresh string (2026-08-24), only within-note movement — finger glides / meend — triggers it; the dip follows the sounding-pitch slew, so drift/vibrato never do. 0 = off."),
            ...
            ParamSpec("bow_slide_noise", "slide noise", group: "Liveness",
                      0.0, 0.02, 0.008,
                      scope: .perNote,
                      help: "Finger-slide friction noise, ACCELERATION-driven (v2, 2026-08-24): filtered per-note noise injects at the finger termination in the kernel — it circulates the string, combs at the sliding pitch and radiates through the body — driven by the CHANGE of the finger's pitch slew, so it scrapes where the finger starts, stops or turns and stays quiet through a constant-rate meend. (The v1 rode the slew itself and hissed steadily through any slide, unrelated to the gesture — cut by ear and reinstated acceleration-driven the same day.) 6000 ¢/s² drive floor: drift and steady notes never trigger it, and steady notes render bit-exactly with it armed. 10 ms attack / 100 ms release, faded with the note's gate. 0 = silent slides."),
    ...
    ///
    ///  * **kernel scalars** — the change lands only in the 66-element
    ...
    public static let inPlaceKeys: Set<String> = [
        ...
        "bow_glide_dip_rate",
        // slide dulling + accel-driven noise (2026-08-24): kernel scalars
        ...
        // The chromatic bridge (2026-09-02): the same per-row table bake
        // (geometry, taps, level/drive) plus the kernel's per-row contact
        // law, re-pushed after every in-place jt reload.
