...
public enum ParamRegistry {
    ...
    public static let groups: [(name: String, params: [ParamSpec])] = [
        ...
        ("Jawari taraf (modal contact)", [
            ...
            ParamSpec("bow_jt_rad_pin", "pin radiation", group: "Jawari taraf (modal contact)",
                      0.0, 1.0, 0.0, apply: .live,
                      help: "Adds the string's termination force at the pin to the radiated contact force: 0 = contact only (bit-exact); 1 = the full bridge force — restores the linear string tone under the buzz, lifts quiet rows. The bone contact force is what a row radiates today (the buzz, ∝ penetration^1.3, so a barely grazing string barely speaks); the pin force T·∂u/∂x|L is the string's own linear pull on the bridge, comb-free and flat in k, and a real bridge feels both. Summed ahead of the row's DC blocker, slewed ~40 ms so a bound axis sweeps it smoothly. Applies live."),
