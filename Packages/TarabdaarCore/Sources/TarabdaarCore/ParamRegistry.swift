...
public enum ParamRegistry {
    ...
    public static let groups: [(name: String, params: [ParamSpec])] = [
        ...
        ("Jawari taraf (modal contact)", [
            ...
            ParamSpec("bow_jt_drive_term", "termination drive", group: "Jawari taraf (modal contact)",
                      0.0, 1.0, 0.0, apply: .live,
                      help: "Where the played string's bridge force ENTERS each sympathetic string. 0 = the fitted 0.90 L tap, which carries a |sin(k·0.9π)| comb — modes 10 and 20 are never charged at all. 1 = the termination itself: the rows share the bridge with the played string, so a moving end excites mode k through the mode slope there, ∝ (−1)^k·k — no null anywhere, the high partials get the drive the tap's comb withheld, and the sign convention matches the pin-force radiation term the same rows already radiate. The shape is ENERGY-matched per row against the tap (same total drive energy over the row's modes), so the slope only redistributes the drive up the mode stack instead of adding level — mode 1 falls, the cluster rises, and the taraf ends up quieter and brighter than the fitted tap. 0 = bypass, bit-exact. Applies live, slewed ~40 ms per row."),
            ParamSpec("bow_jt_couple", "bridge coupling", group: "Jawari taraf (modal contact)",
                      0.0, 0.15, 0.0, apply: .live,
                      help: "Two-way coupling: how much of the sympathetic web's OWN bridge force pushes back on the bridge. The drive is one-way without it — the played string charges the rows and never feels them. Here each row's real bridge load (its contact force on the bone plus the pull at its pin, in newtons) is summed and added back into the played strings' bridge force, so it drives the body, returns into every played string, AND becomes part of what charges every row on the next tick: the web feeds itself through the bridge, which is how a real taraf blooms and how the played string loses energy into it. 1 would be the full physical load; the range stops at 0.15 because the loop (string → bridge → rows → bridge → string) turns divergent at 0.22 under a hard-bowed tonic, and this is a third under that. Even at the top the web is nearly lossless — a 4 s ring loses ~3 dB where the uncoupled one loses 31 — so treat the top of the range as a very long ring, not a safe default. 0 = off, bit-exact. Applies live, slewed ~40 ms."),
