...
One panel per insert point — **Voice → Taraf**, **Voice**, **Taraf**, **Global** — each with a **graphic EQ** (10 vertical ±12 dB faders, 31.5 Hz–16 kHz; double-click a fader to zero it) and a **reverb** (Bigverb/Room picker + mix/size/cutoff; double-click a label to reset). Toggles glide, so switching FX never clicks. A dot by the title marks an active point; **Reset** returns the point to all-off. Every control is a plain `fx_*` registry parameter, so the Parameters tab, presets, tilts and composites see the same knobs. See [FX](fx.md).
