...
- Tap legato for fast phrases (voice hand-off with no re-articulation)
...
- Tilt-controlled performance (3 calibrated tilt axes, bound on the Mac to composites or single parameters)
- Playing scale + fret layout + tilt bindings edited on StarpadMac and synced to the iPad over USB-MIDI SysEx; the iPad is perform-only
