...
- **`DebouncedParamFlush`** (`DebouncedParamFlush.swift`) — the rebuild funnel behind every off-main parameter path: last value per key, one debounced main-thread flush.
