...
TarabdaarMacApp.swift
  └── MacMainWindow
        ...
        └── Views/  (LiveVisualizer, Strings, FretPad, TiltControls,
                     Parameters, BowTransforms, FX, Setup, Scope, TarafScope)
```

The Joy-Con path is split by platform: `JoyConReport`, the `JoyConTransport` protocol, `JoyConMapper` (button edges, stick calibration), `JoyConNYXIReport` / `JoyCon2ReportDecoder` (vendor telemetry and standard BLE input decoders) and `JoyConFusion` (attitude) live in TarabdaarCore and are unit-tested; `JoyConInput` (Mac) coordinates the three Mac-only transports (GameController alias, IOHID full mode, Joy-Con 2 BLE GATT). The BLE transport opens the NYXI console session and routes its standard input into the shared button, stick and motion funnels. Pre-session command-channel telemetry remains a button/stick fallback; ordinary acknowledgements never become input.
...
3. **Tilt and strike (60 Hz).** `NoteManager`'s tick writes the raw tilt into the same `OutboundPlayState`; `MotionManager` supplies the Strike envelope, so everything rides each frame atomically.
