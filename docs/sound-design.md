...
- **The sitar-twang axis** (`bow_twang`, `.live`, default 0 = byte-exact). A grazing jawari wrap on the played strings' own bridge that gives a staccato note the sitar's buzzy high-harmonic cluster through its ring.
...
- `bow_cpl_*` ("Taraf coupling", `bow_cpl_z` 0 = byte-null): a SILENT bridge-coupling layer — one buzz-free comb per tarab row on the passive junction, so the played strings feel the taraf as a two-way load.

Mechanism and measurements: [Sarangi](sarangi.md).

### The formula body

Modal resonators derived from physical scalars (no FIR/fingerprint artifacts): nine signature modes (55–250 Hz) plus the **formant forest** tail — `bow_body_tail_*`: 32 modes, 280–6500 Hz, Q 30, mobility 0.4, radiation 2.5, radiated ripple std ±3.8 dB — so every harmonic sweeps through FIXED peaks and valleys during meend, the body slope the drift shimmer works against. The tail's bridge-load side is deliberately light (admittance max 1.89), so loop cap, stability and wolf behaviour are those of a flat body. All six `bow_body_tail_*` keys are Parameters-tab rows (rebuild tier); `tail_rad` 0.017 ≈ a flat body.

### Stereo

ONE width law: `bow_st_width` (0.2) — the whole instrument (voice, taraf wash, drones, bow noise) heard from two observation points via a diffuse-field difference bank per bus. Lows stay identical in L/R, the upper spectrum decorrelates like a real instrument's, zero net lean, mono fold-down bit-identical, plus the width-decorrelated room (`Reverb.processMonoStereo`). `bow_st_spread` / `bow_st_played` are disarmed legacy pans kept for A/B. Details: [Sarangi](sarangi.md).
...
- **A hard shake can strike it** (`ctl_strum_thresh`, default 127 = off): the iPad's strike envelope crossing the threshold triggers as an L press and releases when it falls back below, with a 100 ms retrigger cooldown (the L button ignores the cooldown).
...
They save and load as **one `TarabdaarPreset` document** — one preset is one rig (`Packages/TarabdaarCore/.../PresetDocument.swift`; data, not UI, hence the package) via `AppController.capturePreset(name:)` / `applyPreset(_:)`, driven from the Parameters-tab toolbar (`PresetToolbar`). A preset carries every section: the sarangi document, the physics overrides, `paramValues`, the composites and `tiltMapping`. Every section is optional, so a partial file applies exactly the sections it carries, and old bare-`InstrumentState` files load through `TarabdaarPreset.decode`'s legacy fallback.
...
**Kin notes and `bow_jt_norm`.** The notes that get near the ceiling are the KIN notes — a hard-struck unison Sa/Pa adds the played voice, the jt ring and the coupling return coherently (~+4 dB peak over a non-kin degree, more under high expression). The musical fix is **`bow_jt_norm`** (t60-response normalization — the long-ring Sa/Pa anchor rows charge hotter than the shortened crowd; **0.6 measures near-even across degrees**, 1.0 overshoots and the crowd wins); the limiter is the safety net behind it. For the ACCUMULATION side — the anchors remembering a whole phrase and blooming +8…+12 dB on the next kin note — **`bow_jt_gov`** is the per-row charge governor ([Sarangi](sarangi.md)): norm evens the driven level per strike, the governor caps what a phrase piles up.

**Voice↔taraf balance + the taraf compressor.** Two `.live` levers at the SPLIT-BUS merge — the same point as the FX voice/taraf inserts and the iPad volume-readout meter tap (the `bow_poly_process3` split path's `bus + bus` sum is bit-exact against the fused path; `BusMeterTests`):
...
- **`bow_jt_comp_*`** (thresh/ratio/atk_ms/rel_ms; thresh 0 = off, bit-exact): a feed-forward compressor on the jt bus ONLY — the voice bus repeats bit-identically under compression. Threshold is in CALIBRATED OUTPUT units (× the build-time trim, the scale the limiter and the iPad meter speak — it does not move with `bow_gain`); the stock taraf rides ~0.01–0.1. Instant-attack envelope, `atk_ms` gain slew toward reduction (lets the jawari strike transient through), `rel_ms` recovery, linked mid/side so the width law's fold-down stays consistent. Where norm evens the per-strike drive and the governor caps the pile-up at the CAUSE, the compressor holds the radiated result; at ratio 20 it is a taraf limiter.

**The voice-relative taraf cap** (`bow_jt_cap` / `bow_jt_cap_ratio` / `bow_jt_cap_bus`) is the runaway-bloom lever: with high `bow_jt_evolve` the web can feed itself and bloom LOUDER than the played voice, and a fixed-threshold compressor can't follow a phrase's dynamics. The cap side-chains its ceiling from the VOICE bus itself — an instant-attack peak envelope with a slow ~1.2 s-τ release (≈7 dB/s), so a string may ring on after a note and decay more slowly than the voice but never PEAK above what the voice reached.
...
- **Applied INSIDE the kernel, string by string** (`bow_poly_jt_set_cap`): the render loop records the voice peak envelope beside the jt drive (sync buffer and async ring alike — state only, byte-null unarmed); the tick scheduler converts it to a per-tick ROW ceiling (voice env × ratio ÷ the jt output gain), and each row runs its own 150 ms peak envelope and gain (3 ms toward reduction / 120 ms recovery) on its radiated output — a pure output gain after the physics, so the string's ring, the charge governor and the quiescence gate all see the un-capped string. Per-row state is worker-owned; an arm edge bumps a generation counter and each row resets itself on its next tick (no control-thread array writes under running workers). The strings sum AFTER the cap, so the whole web can still stand above one string's ceiling.
- **`bow_jt_cap_bus`** blends the SCOPE: 0 = per string (the default); 1 = per taraf — the summed web is held against the same ceiling in the kernel's hold walk (`jt_cap_bus`, one gain on mono and side); between, each row removes hard·(1−bus) of its own overshoot, then the sum removes hard·bus of what remains. The bus end leaves LESS ring at the same ratio, so the knob runs from "one hot string is held alone" to "one hot string ducks the whole web" — tilt-bindable.
...
`TarafCapTests` pins byte-null at hard 0, taraf-only reduction, monotonicity in both knobs, the worker-pool path and the scope blend.
