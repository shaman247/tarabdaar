...
| `bow_body_tail_n` | formant modes | 0 … 48 | 32 | global | rebuild | Diffuse mid/high mode forest between the tail corners — the FIXED body formants the harmonics sweep through during a glide (the cue that separates a real slide from a pitch-shifted tone; without them the radiated transfer above the signature modes measured ±0.3 dB flat — 2026-08-24). √n-normalized: more modes = denser, individually shallower structure at held total power. 0 = flat feedthrough only. |
...
| `bow_body_tail_y` | formant mobility | 0 … 1.5 | 0.4 | global | rebuild | Bridge-load side of the formant modes. Kept modest: the shipped 0.4 leaves the admittance max (and the loop cap) unchanged — raising it far invites wolves. |
| `bow_body_tail_rad` | formant radiation | 0 … 8 | 2.5 | global | rebuild | How loudly the formant forest radiates against the flat floor. Shipped 2.5 ≈ ±4 dB ripple std across 250–6500 Hz (extremes ~26 dB) — real-body territory; the pre-2026-08-24 0.017 was inaudibly flat. |
...
| Key | Name | Range | Default | Scope | Timing | Description |
|---|---|---|---|---|---|---|
| `bow_jt_gain` | level | 0 … 3 | 0.3 | global | in-place | Output mix of the jawari web (the gap-law calibration landed at 0.3). Glides ~40 ms inside the kernel (2026-09-03): the bridge-force radiation carries the contact pulse train's low-frequency swing, and a stepped gain on it clicked. |
...
| `bow_jt_apex` | graze depth | 2e-06 … 5e-05 | 1e-05 | global | in-place | Bone protrusion. The evolution lives at the grazing knee — deep press linearizes (sparkle only), too shallow never engages. (Two bridges since 2026-09-02: this group is the RAGA set's bridge plus the web-wide taraf controls; the CHROMATIC set has its own bridge group below.) |
| `bow_jt_zone` | contact zone (m) | 0.002 … 0.02 | 0.006 | global | in-place | Length of the bone the string can touch, in metres — the flat of the jawari. The fitted 6 mm concentrates the contact grid on the active region (J8z6); wider = a flatter, more open jawari whose wrap spreads along the bone (longer, more diffuse buzz), narrower = a sharper knee. |
...
| `bow_jt_comp_thresh` | comp threshold | 0 … 0.3 | 0 | global | live | Taraf compressor threshold, in the CALIBRATED output units the master limiter and the iPad volume readout speak (≈ linear dBFS at master gain 1 — it does not move with bow_gain). The stock taraf rides ~0.01–0.1 (−40…−20 dBFS), so ~0.02–0.05 starts holding the ring's swells while single strikes pass. 0 = off, bit-exact. Applies live. |
| `bow_jt_comp_ratio` | comp ratio | 1 … 20 | 4 | global | live | Compression slope above the threshold: 1 = off (unity), 4 = classic leveling, 20 ≈ a taraf limiter — the sympathetic wash sits at a near-constant level however hard the phrase charges it. Applies live. |
| `bow_jt_comp_atk_ms` | comp attack (ms) | 0 … 100 | 5 | global | live | Gain-reduction attack: how much of a strike's first milliseconds passes before the compressor holds the ring. 0 = instant (clamps the transient too); longer keeps the jawari sparkle and compresses only the sustained wash. Applies live. |
| `bow_jt_comp_rel_ms` | comp release (ms) | 20 … 1000 | 150 | global | live | Gain recovery after the taraf level falls back under the threshold. Short pumps audibly on a ringing wash; long ducks the tail after every hot phrase. Applies live. |
...
| `bow_jtc_drive` | drive | 0.001 … 0.3 | 0.03 | global | in-place | Bridge-force coupling INTO the chromatic strings — their graze operating point, independent of the raga bridge's. On the instrument the chromatic set passes through the main bridge, closest to the played strings: a touch more drive than the side bridges is the physical picture. |
| `bow_jtc_apex` | graze depth | 2e-06 … 5e-05 | 1e-05 | global | in-place | Bone protrusion of the chromatic bridge — its own grazing knee (the deep-substep threshold and the evolution map follow it per row). |
...
| `bow_jtc_zone` | contact zone (m) | 0.002 … 0.02 | 0.006 | global | in-place | Length of the chromatic bridge's bone the string can touch (the flat of its jawari), in metres — wider = a more open, diffuse buzz. |
| `bow_jtc_radius` | bone radius (m) | 0.05 … 2 | 0.3 | global | in-place | Curvature radius of the chromatic bridge's bone, in metres — large = nearly flat (the wide open jawari), small = rounded. |
| `bow_jtc_alpha` | contact law | 1 … 2 | 1.3 | global | in-place | Contact stiffness exponent of the chromatic bridge (per row in the kernel). 1.5 = Hertz (the fast sqrt path); the shipped 1.3 is the fitted raga value. |
...
| `bow_jtc_hcb` | contact damping | 1 … 40 | 8 | global | in-place | Hysteretic damping of the string–bone contact on the chromatic bridge (per row in the kernel). More = softer buzz transients. |
| `bow_jtc_fhf` | damping corner (Hz) | 800 … 12000 | 4000 | global | in-place | Corner of the chromatic strings' per-mode f² damping law — LOWER = warmer. Their own string material, independent of the raga set's. |
| `bow_jtc_bst` | inharmonicity | 0 … 0.001 | 0.0002 | global | in-place | Stiffness stretch of the chromatic strings' upper partials (steel-wire dispersion); lower = more harmonic top. |
...
| `ctl_glide_rate` | glide rate (st/s) | 2 … 200 | 40 | per-note | live | Base speed of a queued glide, in semitones per second, when the note being left has been RELEASED. A 12-semitone glide at 40 st/s takes 0.3 s. Bindable — the legacy keyboard's tilt-driven glide speed, reborn on the registry. |
