...
| `bow_jtc_alpha` | contact law | 1 … 2 | 1.3 | global | in-place | Contact stiffness exponent of the chromatic bridge (per row in the kernel). 1.5 = Hertz (the fast sqrt path); the default 1.3 matches the fitted raga bridge. |
...
| `bow_cpl_damp` | HF damping | 0 … 1 | 0.019 | global | rebuild | Frequency-dependent loop damping of the coupling combs — upper partials couple and die faster, matching real string damping. Default 0.019. |
| `bow_cpl_bright` | bandwidth | 0 … 1 | 0.8 | global | rebuild | Coupling bandwidth: loop low-pass corner 1.4–7.4 kHz; above it the taraf stops exchanging energy with the bridge. Default 0.8 ≈ 6.2 kHz. |
| `bow_cpl_inharm` | inharmonicity | 0 … 0.6 | 0.1 | global | rebuild | Stiffness dispersion of the coupling combs (steel-string upper partials run sharp), matching the jt rows' inharmonicity in spirit. Default 0.1. |
...
| `bow_st_spread` | taraf pan spread (legacy) | 0 … 1 | 0 | global | rebuild | Fixed per-row staging of the jawari rows around the tonic (spread·sin(2π·pitch class)) — an alternative placement to the instrument-width law, for A/B against bow_st_width. 0 = off (default). Mono fold-down invariant. |
| `bow_st_played` | bow-noise pan spread (legacy) | 0 … 0.5 | 0 | global | rebuild | Per-voice placement of the bow noise (the played string's position on the bridge) — for A/B against bow_st_width. 0 = off (default). Mono fold-down invariant. |
